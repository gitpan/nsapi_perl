...
B<libperl> and B<tracelog>.  B<libperl> may be specified in order to
have properly-functioning dynamic loading of extension modules; see
L</DYNAMIC LOADING OF EXTENSIONS> for details. B<tracelog> may be used
to enable nsapi_perl tracing; see L</DIAGNOSTICS> for full details.
...
adding the B<libperl> argument to the nsapi_perl_init() function call
in obj.conf, like this:

 Init fn="nsapi_perl_init"
      init-script="/home/benjamin/foobar.pl"
      libperl="/path/to/libperl.so"

Here, F</path/to/libperl.so> should be the full path to the (shared)
Perl library.  To find the right value of F</path/to/libperl.so> on
your system, try this statement:

 perl -MConfig -e 'print "$Config{archlib}/CORE\n"'

This should print out the name of the directory containing
F<libperl.so>.

The presence of a B<libperl> argument in the call to nsapi_perl_init
causes nsapi_perl to call dlopen(3) on F</path/to/libperl.so>, passing
...
may be others too that I am not aware of.  A B<libperl> argument on a
...
nsapi_perl still does not work (or at least not very well) with Perls
that use a static Perl library (such as libperl.a) rather than a
dynamic perl library (libperl.so).  At this point the best workaround
this bug is to recompile Perl so that is uses a dynamic Perl library.
Hopefully this restriction will soon be removed.
